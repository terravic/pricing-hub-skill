...
Whenever any skill task finishes (contract ingestion, single claim .x12 parsing, golden dataset parity verification, or pipeline migration monitoring), the skill automatically generates `output/dashboard.html` (and synchronizes `dashboard.html` and `src/ui/dashboard.html`) with the run's data as a primary skill output, providing an immediate visual inspection surface.
...
- **Primary Skill Output**: Open or click `output/dashboard.html` generated by any task execution.
- **Interactive Artifact Panel**: View and interact directly with `dashboard.html` inside your agent harness UI.
- **Direct Workspace File Access**: Open `dashboard.html`, `output/dashboard.html`, or `src/ui/dashboard.html` directly in any web browser.
